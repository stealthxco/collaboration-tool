...
export class RBACService {
   ...
   * Update permission
   */
  async updatePermission(permissionId: string, data: Partial<CreatePermissionData>): Promise<Permission> {
   ...
   * Assign role to user
   */
  async assignRoleToUser(userId: string, roleName: string): Promise<UserRole> {
   ...
   */
  async bulkAssignRoles(userIds: string[], roleNames: string[]): Promise<void> {
    const roles = await this.prisma.role.findMany({
      where: {
        name: { in: roleNames }
      }
